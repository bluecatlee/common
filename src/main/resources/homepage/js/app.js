   ...
   var num=0;
    $(".add-mold01").click(function(){
    	if($(this).prev("ul").find("li").size() < 3){
            num+=1;
    		$(this).prev("ul").append('<li>满<span><input class="form-control input-xs text-center required isNumber discount" id="fullPrice'+num+'" name="fullPrice" type="text"/></span>元，打<span><input name="fullbuyDiscount" id="fullbuyDiscount'+num+'"class="form-control input-xs text-center required zeroOne" type="text"/></span>折' +
            '<button class="btn btn-default btn-sm del-mold" type="button">删除本级促销</button></li>');
    	}
    });
